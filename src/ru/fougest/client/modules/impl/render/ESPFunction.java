...
public class ESPFunction extends Function {
    ...
    private void renderPlayerElements(MatrixStack stack) {
        ...
        for (Map.Entry<Vector4d, PlayerEntity> entry : positions.entrySet()) {
            ...
            if (elements.get(2)) {
                BloomHelper.registerRenderCall(() -> {
                    Fonts.rubikSemiBold[10].drawStringWithOutline(stack, healthText, position.x - 3 - size.getValue().floatValue() - Fonts.rubikSemiBold[10].getWidth(healthText), position.y + (height * (1 - player.animationPerc)) + 1, colors.x);
                });
                Fonts.rubikSemiBold[10].drawStringWithOutline(stack, healthText, position.x - 3 - size.getValue().floatValue() - Fonts.rubikSemiBold[10].getWidth(healthText), position.y + (height * (1 - player.animationPerc)) + 1, colors.x);
            }
            if (elements.get(3)) {
    ...
    private void renderEffects(PlayerEntity player,
        ...
        for (int i = 0; i < effectCount; i++) {
            ...
            Fonts.rubikSemiBold[12].drawStringWithShadow(matrices, effectString, endX + 2.5f, y - 2 + ((i + 1) * 8), -1);
        }
    }
    ...
    private void renderArmorAndEnchantment(List<ItemStack> stacks,
        ...
        for (ItemStack stack : stacks) {
            ...
            int i = 0;
            for (String s : lines) {
                Fonts.rubikSemiBold[12].drawCenteredString(matrixStack, s,
                        center,
