...
public class JumpCircleFunction extends Function {
    ...
    private void updateCircles() {
        for (Circle circle : circles) {
            circle.factor = AnimationMath.lerp(circle.factor, 1f, 4);
            circle.alpha = AnimationMath.lerp(circle.alpha, 0, 4);
        }
        if (circles.size() >= 1)
    ...
    private void renderCircles() {
        setupRenderSettings();
        for (Circle circle : circles) {
            drawJumpCircle(circle, circle.factor, circle.alpha, 14);
        }
